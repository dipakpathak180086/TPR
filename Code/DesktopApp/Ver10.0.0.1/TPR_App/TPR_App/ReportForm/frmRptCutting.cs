    ...
    public partial class frmRptCutting : Form
    {
        ...
        private void btnGet_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                DataTable dt = oDal.GetCuttingReportData(dtpFromDate.Text, dtpToDate.Text, ModelNo, txtLotNo.Text.Trim(), "0", "1",LineNo);
                dgv.DataSource = dt;
        ...
        private void ChangeGridColor()
        {
            try
            {
                for (int i = 0; i < dgv.Rows.Count; i++)
                {
                    ...
                    if (dgv.Rows[i].Cells["QAPending"].Value.ToString().ToUpper() == "YES")
