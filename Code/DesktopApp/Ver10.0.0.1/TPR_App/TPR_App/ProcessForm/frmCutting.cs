    ...
    public partial class frmCutting : Form
    {
        ...
        private void frmModelMaster_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                lblMessage.Text = "";
                //If User has right to change manual charge pallet then enable it 
                if (ClsGlobal.IsCuttingManualEnable)
                {
                    txtChargeNo.ReadOnly = false;
                    txtChargeNo.BackColor = Color.White;
                    txtPalletNo.ReadOnly = false;
                    txtPalletNo.BackColor = Color.White;
                }
                LoadData();
                txtChargeNo.Focus();
                lblWelcome.Text = "Logged In User : " + ClsGlobal.UserName;
        ...
        private void pbIncreaseCharge_Click(object sender, EventArgs e)
        {
            try
            {
                //ReSet Charge/Pallet
                int ChargeNo = int.Parse(txtChargeNo.Text.Trim());
                ChargeNo++;
                txtChargeNo.Text = ChargeNo.ToString();
                txtPalletNo.Text = "1";
            }
            catch (Exception ex)
            {
        ...
        private void btnNext_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (ValidateInput())
                {
                    ...
                    oCutting.ChargeNo = Convert.ToInt32(txtChargeNo.Text.Trim());
                    oCutting.PalletNo = Convert.ToInt32(txtPalletNo.Text.Trim());
                    oCutting.LineNo = ClsGlobal.LineNo;
            ...
            if (ClsGlobal.CuttingDefectAutoClose)
            {
                if (oCutting.IsMixedTrolley)
                {
                    ...
                    else
                    {
                        ...
                        txtTotalQty.Text = "";
                        lblNgQty.Text = "0";
                        //Set ChargeNo/PalletNo
                        int PalletNo = int.Parse(txtPalletNo.Text.Trim());
                        PalletNo++;
                        txtPalletNo.Text = PalletNo.ToString();
                    }
                }
                else
                    btnReset_Click(sender, e);
        ...
        private void btnReset_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                pbIncreaseCharge.Enabled = false;
                _ListCutting.Clear();
                Clear();
                LoadData();
                txtChargeNo.Focus();
        ...
        private void AddMixedTrolleyData(int NewPendingQty)
        {
            try
            {
                ...
                _ListCutting.Add(new Cutting
                {
                    ...
                    ChargeNo = oCutting.ChargeNo,
                    PalletNo = oCutting.PalletNo,
                    LineNo = oCutting.LineNo,
                ...
                if (MessageBox.Show("Do you want to print trolley card ?", "Confirm", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    /*
                     * In Case Of Mix Trolley of different date then Auto Generate charge no will return
                     * Wrong entry becuase it will return max charge no, we wil change charge no to as lat lot no charge no
                   */
                    int chargeNo = _ListCutting[_ListCutting.Count - 1].ChargeNo;
                    int palletNo = _ListCutting[_ListCutting.Count - 1].PalletNo;
                    ...
                    for (int i = 0; i < _ListCutting.Count; i++)
                    {
                        ...
                        //Update Charge No
                        _ListCutting[i].ChargeNo = chargeNo;
                        _ListCutting[i].PalletNo = palletNo;
                    }
        ...
        private void Clear(bool IsDateClear = true)
        {
            try
            {
                lblMessage.Text = "";
        ...
        private void LoadData(bool IsPreviousMonth = false, string PreviousMonth = "")
        {
            try
            {
                lblMessage.Text = "";
                DataSet ds = oDal.GetCuttingData(IsPreviousMonth, PreviousMonth);
                ...
                if (ds.Tables[2].Rows.Count > 0)
                {
                    ...
                    //Get Autogenerate ChargeNo,PalletNo
                    GetChargePalletNo();
                }
                else
        ...
        private void GetChargePalletNo()
        {
            try
            {
                lblMessage.Text = "";
                DataTable dt = oDal.GetChargePalletNo(txtModelNo.Text.Trim(), dtpDate.Text, ClsGlobal.LineNo);
                int ChargeNo = Convert.ToInt32(dt.Rows[0]["ChargeNo"]);
                if (ChargeNo == 0) ChargeNo++;
                txtChargeNo.Text = ChargeNo.ToString();

                int PalletNo = Convert.ToInt32(dt.Rows[0]["PalletNo"]);
                if (PalletNo >= 3)
                {
                    if (MessageBox.Show("Pallet no reach to 3, do you want to reset pallet and increase charge no", "Confirm", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                    {
                        ChargeNo++;
                        PalletNo = 1;
        ...
        private void dtpDate_ValueChanged(object sender, EventArgs e)
        {
            try
            {
                //In case month change and user want to use old month, mostly in case of last date of month 
                if (dtpDate.Value.Month < DateTime.Now.Month)
                {
                    chkMixedTrolley.Enabled = true;
                    chkMixedTrolley.Checked = false;
                    pbIncreaseCharge.Enabled = false;
                    _ListCutting.Clear();
                    Clear(false);
                    LoadData(true, dtpDate.Value.ToString("MMM-yyyy"));
                    txtChargeNo.Focus();
                }
                else
                    GetChargePalletNo();
